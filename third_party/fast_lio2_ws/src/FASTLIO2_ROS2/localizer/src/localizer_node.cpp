...
public:
    LocalizerNode() : Node("localizer_node")
    {
        ...
        // ICP may take longer than the TF tolerance on machines using software
        // rendering for RViz. Keep sensor ingestion and TF publication in
        // independent callback groups so an ICP update cannot freeze the pose.
        ...
        m_cloud_sub.subscribe(this, m_config.cloud_topic, qos.get_rmw_qos_profile(), sensor_options);
        m_odom_sub.subscribe(this, m_config.odom_topic, qos.get_rmw_qos_profile(), sensor_options);
        ...
        m_alignment_timer = this->create_wall_timer(
            10ms, std::bind(&LocalizerNode::timerCB, this), m_localization_group);
        m_tf_timer = this->create_wall_timer(
            20ms, std::bind(&LocalizerNode::broadcastLatestTF, this), m_tf_group);
    }
    ...
    void syncCB(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg, const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg)
    {
        ...
        CloudType::Ptr latest_cloud = std::make_shared<CloudType>();
        pcl::fromROSMsg(*cloud_msg, *latest_cloud);

        std::lock_guard<std::mutex>(m_state.message_mutex);
        m_state.last_cloud = latest_cloud;
    ...
    void broadcastLatestTF()
    {
        ...
        // map -> lidar is the localization correction between two world
        // frames, not the moving lidar/body pose.  Keep this correction
        // available at the current ROS time.  Stamping it with the point-cloud
        // time made every newly planned Nav2 path newer than this transform
        // whenever FAST-LIO2 was about one scan behind, so the controller
        // repeatedly rejected the path as "Transform data too old".
        const builtin_interfaces::msg::Time current_time = this->get_clock()->now();
        sendBroadCastTF(current_time, offset_r, offset_t);
    }
...
private:
    ...
    rclcpp::TimerBase::SharedPtr m_alignment_timer;
