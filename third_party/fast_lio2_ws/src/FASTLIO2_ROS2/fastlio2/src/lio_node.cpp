...
public:
    LIONode() : Node("lio_node")
    {
        ...
        // Point-cloud processing may take longer than one sensor period. Keep
        // IMU ingestion, lidar conversion and state estimation in independent
        // callback groups so processing cannot make the navigation pose stale.
        m_imu_callback_group = this->create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
        m_lidar_callback_group = this->create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
        m_processing_callback_group = this->create_callback_group(
            rclcpp::CallbackGroupType::MutuallyExclusive);
    ...
    bool syncPackage()
    {
        std::scoped_lock buffer_lock(
            m_state_data.imu_mutex, m_state_data.lidar_mutex);
